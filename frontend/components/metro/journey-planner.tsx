...
export default function JourneyPlannerWidget() {
  ...
  return (
    <div className="w-full bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xl space-y-6 relative">
      <div className="absolute top-0 right-0 w-72 h-72 bg-amber-500/5 rounded-full blur-3xl pointer-events-none rounded-r-3xl" />

      {/* Header Widget Bar */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-100 relative z-10">
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-2xl bg-amber-100 border border-amber-300 flex items-center justify-center shadow-sm shrink-0">
            <Navigation className="w-5 h-5 text-amber-600" />
          </div>
          <div>
            <h3 className="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Plan Your Metro Journey</h3>
            <p className="text-xs text-slate-500 font-semibold mt-0.5">Real-time route calculation & fare matrix</p>
          </div>
        </div>
        <span className="px-3.5 py-1.5 rounded-full text-xs font-black bg-emerald-50 text-emerald-800 border border-emerald-300 flex items-center gap-1.5 shadow-sm shrink-0">
          <Sparkles className="w-4 h-4 text-emerald-600" /> Priority Corridor Active
        </span>
      </div>
      ...
      {/* Selection Form */}
      <form onSubmit={handlePlan} className="space-y-5 relative z-10">
        <div className="bg-slate-50/90 p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-4 hover:border-amber-300 transition">
        ...
        {errorMsg && (
          <div className="flex items-center gap-2 text-xs text-rose-900 bg-rose-50 border border-rose-300 px-4 py-3 rounded-xl font-bold">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full h-13 sm:h-14 rounded-2xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-base flex items-center justify-center gap-2 shadow-lg shadow-amber-500/25 transition transform hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50"
        >
